...
const int waterIntSP = 10000;                    //auto water interval [ms]
...
int ctlmode = 0;                    //fully automatic closed loop = -1; time trigger open loop = 1 [default]; off = 0;
int soilUL = 700;                   //upper limit for soil moisture 4-923
int soilLL = 400;                   //lower limit for soil moisture 4-923
int moisturePV;
int lastPV;
...
void hydrate(int mode){ //int mode = {0 - 0ff; -1 - Auto (ctlmode = -1); 1 - Timer (ctlmode = 1);}
  ...
  switch (mode) {
    ...
    cnt = 0;
      ...
      Serial.println(String (millis())+"> |PV-"+moisturePV+"|AUTO| - hydration complete|" );
    ...
    case 1:
      ...
      hydrateTotal = hydrateTotal + doseSP;
      waterStart =  waterIntSP + millis(); 
      Serial.println(String (millis())+"> |PV-"+moisturePV+"|TIMER| - hydration complete|" );
